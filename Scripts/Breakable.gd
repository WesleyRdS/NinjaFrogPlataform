	...
	if fruits <= 0:
		...
		for h in range(divY):
			for w in range(divX):
				...
				rigid.apply_impulse(Vector2.ZERO, Vector2(rand_range(-50, 50), rand_range(-100, -150))) #fisica que pede dois parametros respectivamente o deslocamento e o impulso
		queue_free()
	else:
		create_fruit()
		$anim.play("hit")
